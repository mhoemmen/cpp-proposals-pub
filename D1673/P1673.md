...
# Revision history
  ...
  * Revision 13 - running revision for LWG review
    ...
    * Make sure that `conjugated` is trivial if _`conj-if-needed`_ would be.
...
[2]{.pnum} *Returns:*
  ...
  * [2.1]{.pnum} `mdspan<typename A::element_type, Extents, Layout, A>(a.data_handle(), a.mapping(), a.accessor().nested_accessor())` if `Accessor` is a specialization of `conjugated_accessor`;

  * [2.2]{.pnum} otherwise, `a` if `remove_cvref_t<ElementType>` is an arithmetic type;

  * [2.3]{.pnum} otherwise, `a` if `conj(declval<ElementType>())` is ill-formed, with overload resolution performed in a context that includes the declaration `template<class T> T conj(const T&) = delete;`;
